...
func cloudCmd() *cobra.Command {
	...
	root.AddCommand(cloudLoginCmd(), cloudLogoutCmd(), cloudConnectCmd(), cloudDisconnectCmd(), cloudInstallAgentCmd(), cloudUninstallAgentCmd(), cloudServicesCmd())
	return root
}

// cloudServicesCmd agrupa lo que un desarrollador ve del lado de Cloud del
...
// concreta, no de "servicios" como concepto de Cloud. Vivía antes en
// `asterion services` de nivel superior; se movió acá para que ese nombre
// corto quede libre para las unidades systemd de ESTA máquina (ver
// services.go), que no necesitan sesión — a diferencia de esto, que sí.
func cloudServicesCmd() *cobra.Command {
	root := &cobra.Command{
		Use:   "services",
		Short: "Servicios registrados en un proyecto de Asterion Cloud (réplicas de plugins agrupadas por nombre)",
	}
	root.AddCommand(cloudServicesListCmd())
	return root
}

func cloudServicesListCmd() *cobra.Command {
