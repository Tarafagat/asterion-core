...
// asterion-lab y asterion-plugin-contract viven en su propio módulo cada
// uno, hermanos de este (carpetas al lado de asterion-core/) — ninguno está
// publicado en ningún registry, así que se referencian por ruta local,
// mismo criterio que asterion-shared del lado Python (editable install
// consumido por asterion-core/backend-core y asterion-cloud/backend).
